...
<NumberInput
  ...
  // Render Text Panel Content (Desktop - with accordions)
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1 flex flex-row flex-wrap gap-1 px-1">
                  ...
                  {textSections.map((section, index) => (
                    <SubsectionButton 
                      key={section.id}
                      title={`${index + 1}: ${section.text?.slice(0, 8) || 'Empty'}${section.text?.length > 8 ? '...' : ''}`}
                      onClick={() => openDialog(`text-section-${section.id}`)} 
                    />
                  ))}
